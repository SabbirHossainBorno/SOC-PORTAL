...
export default function RosterSchedulePage() {
  ...
  // Fetch team members who have data in the roster
  const fetchTeamMembers = async () => {
    try {
      console.debug('Fetching roster columns with data for:', { month: filters.month, year: filters.year });
      const response = await fetch(
        `/api/user_dashboard/roster/roster_columns?month=${filters.month}&year=${filters.year}`
      );
      ...
      if (result.success) {
        ...
        console.debug('Roster columns fetched successfully:', result.data);
      } else {
        console.error('Failed to fetch roster columns:', result.message);
        setTeamMembers([]);
      }
    } catch (error) {
  ...
  const summaryData = useMemo(() => {
    if (rosterData.length === 0 || teamMembers.length === 0) return {};
  ...
  const fetchRosterData = async () => {
    try {
      ...
      if (result.success) {
        ...
        // After fetching roster data, fetch the team members who have data
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        // Refresh both roster data and team members
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50 py-6 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Summary Table */}
        {rosterData.length > 0 && teamMembers.length > 0 && (
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          {showModal && selectedDay && (
            <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4 z-50">
              <motion.div 
                ...
                className="bg-white rounded shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden border border-gray-200 flex flex-col"
              >
                {/* Header - Fixed */}
                <div className="flex-shrink-0 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-t">
                  <div className="p-4">
                    <div className="flex items-center justify-between">
                      ...
                      <button 
                        onClick={closeModal}
                        className="bg-white/20 hover:bg-white/30 p-1 rounded-full transition-colors flex-shrink-0"
                      >
                ...
                {/* Footer - Fixed */}
                <div className="flex-shrink-0 border-t border-gray-200 bg-gray-50 rounded-b">
