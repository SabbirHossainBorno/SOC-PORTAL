    ...
    try {
        ...
        // Query for reliability impacted downtime by channel - now including ALL channels
        const queryStr = `
        ...
        ? Math.round((totalReliabilityImpactMinutes / totalAvailableMinutes) * 100) 
        : 0;
        ...
        percentage: totalAvailableMinutes > 0 
            ...
            : 0,
        reliabilityPercentage: totalAvailableMinutes > 0 
            ? Math.max(0, 100 - Math.round((parseInt(row.total_minutes) / totalAvailableMinutes) * 100))
            : 100
        }));
        
        // Sort channels by reliability percentage (highest first) or by impact (lowest impact first)
        channels.sort((a, b) => b.reliabilityPercentage - a.reliabilityPercentage);
        ...
        summary: {
            ...
            mostReliableChannel: channels.length > 0 ? channels[0].channel : 'None',
            leastReliableChannel: channels.length > 0 ? channels[channels.length - 1].channel : 'None',
