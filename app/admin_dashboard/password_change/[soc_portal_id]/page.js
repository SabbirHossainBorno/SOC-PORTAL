// app/admin_dashboard/password_change/[soc_portal_id]/page.js
'use client';
...
export default function PasswordChangePage() {
  ...
  // Get admin data
  useEffect(() => {
    console.log('[useEffect] Fetching admin data...');
    console.log(`[useEffect] Using soc_portal_id: ${soc_portal_id}`);
    
    const fetchAdminData = async () => {
      try {
        console.log(`[fetchAdminData] Calling API: /api/admin_dashboard/admin_info?id=${soc_portal_id}`);
        
        const response = await fetch(`/api/admin_dashboard/admin_info?id=${soc_portal_id}`);
        const data = await response.json();
        
        console.log('[fetchAdminData] API response:', {
          status: response.status,
          data: data
        });
        
        if (response.ok) {
          console.log('[fetchAdminData] Setting admin data:', data);
          setAdminData(data);
        } else {
          console.error('[fetchAdminData] Failed to fetch admin data:', data.error);
          toast.error('Failed to load admin information');
        }
      } catch (error) {
        console.error('[fetchAdminData] Network error:', error);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`/api/admin_dashboard/password_change/${soc_portal_id}`, {
        method: 'PUT',
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded p-4 shadow-md flex items-center">
            ...
            <div>
              <div className="text-xl font-bold">{adminData?.id || 'A01SOCP'}</div>
              <div className="text-xs sm:text-sm mt-1">Admin ID</div>
            </div>
          </div>
