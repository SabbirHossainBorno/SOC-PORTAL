...
export const generateAuditMessage = (oldData, newData) => {
  ...
  // Effective date changes - only show if there's a meaningful change
  const oldEffectiveDate = formatDateForComparison(oldData.effective_date);
  const newEffectiveDate = formatDateForComparison(newData.effective_date);
  
  if (oldEffectiveDate !== newEffectiveDate) {
    messages.push(`Effective Date Changed: ${formatDateForDisplay(oldData.effective_date)} to ${formatDateForDisplay(newData.effective_date)}`);
  }
...
// Validate dates
export const validateDates = (effective_date, revoke_date) => {
  const errors = {};
  const today = new Date().toISOString().split('T')[0];
  
  // Effective date cannot be in the future
  if (effective_date > today) {
    errors.effective_date = 'Effective date cannot be in the future';
  }
  
  // Revoke date cannot be before effective date
  if (revoke_date && effective_date && revoke_date < effective_date) {
    errors.revoke_date = 'Revoke date cannot be before effective date';
  }
