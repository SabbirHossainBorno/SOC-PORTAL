...
const archivePreviousLogFile = () => {
  ...
  const logFile = path.join(logDir, `soc-portal-${yesterday}.log`);
  const archiveFile = path.join(archiveDir, `soc-portal-${yesterday}.log`);
...
const logger = createLogger({
  ...
  format: format.combine(
    format.timestamp({
      format: () => moment().tz('Asia/Dhaka').format('YYYY-MM-DD HH:mm:ss')
    }),
  ...
  transports: [
    ...
    new transports.DailyRotateFile({
      filename: 'logs/soc-portal-%DATE%.log',
